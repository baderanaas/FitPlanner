...
def convert_message_to_dict(message: Any) -> Dict:
    ...
    try:
        if isinstance(message, dict):
            ...
            if "role" in message:
                message["role"] = LANGCHAIN_TO_OPENAI_ROLE.get(
        ...
        if hasattr(message, "type") and hasattr(message, "content"):
            ...
            logger.debug(
                f"Converting LangChain message: {type(message)}, type='{langchain_type}' -> role='{openai_role}'"
            )
        ...
        if hasattr(message, "role") and hasattr(message, "content"):
            ...
            logger.debug(
                f"Converting message with role/content: {type(message)}, role='{original_role}' -> '{mapped_role}'"
            )
        ...
        logger.warning(
            f"Unknown message type {type(message)}, converting to user message"
        )
        return {"role": "user", "content": str(message)}  # Default role
...
def call_openai(state: AgentState, tools: List[Dict]) -> Dict:
    ...
    logger.debug(f"Number of messages in state: {len(state.get('messages', []))}")
    ...
    for i, message in enumerate(state["messages"]):
        ...
        try:
            ...
            if not isinstance(converted_msg["content"], (str, list)):
                logger.warning(
                    f"Converting message content from {type(converted_msg['content'])} to string"
                )
                converted_msg["content"] = str(converted_msg["content"])

            converted_messages.append(converted_msg)
            logger.debug(
                f"Converted message {i}: role={converted_msg['role']}, content_type={type(converted_msg['content'])}"
            )
            ...
            converted_messages.append(
                {"role": "user", "content": f"Error processing message: {str(e)}"}
            )

    logger.debug(f"Successfully converted {len(converted_messages)} messages")
    ...
    try:
        ...
        logger.debug(
            f"Response message content length: {len(msg.content) if msg.content else 0}"
        )
        ...
        if msg.tool_calls:
            logger.debug(f"Response includes {len(msg.tool_calls)} tool calls")
            result["tool_calls"] = [
                {
                    "id": call.id,
                    "type": "function",
        ...
        logger.debug("Successfully created response dictionary")
        return result
...
def call_model(state: AgentState) -> AgentState:
    ...
    logger.debug(
        f"Input state keys: {list(state.keys()) if isinstance(state, dict) else 'Not a dict'}"
    )
    ...
    try:
        ...
        logger.debug(f"Retrieved {len(tools) if tools else 0} tools")
        ...
        logger.debug("OpenAI call completed successfully")
